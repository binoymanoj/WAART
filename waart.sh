...
perform_recon() {
    ...
    if [[ "$URL" =~ ^http:// || "$URL" =~ ^https:// ]]; then
        ...
        if [ -n "$DOMAIN_WOP" ]; then
            echo "[*] Performing amass scan on $DOMAIN_WOP ..."
            amass enum -d $DOMAIN_WOP | tee -a amass.txt
            echo -e "[*] amass scan results stored in amass.txt \n"
        else
            echo "Error: Unable to retrieve the Domain for amass Scan."
        fi

        # Perform subfinder scan
        if [ -n "$DOMAIN_WOP" ]; then
            echo "[*] Performing subfinder scan on $DOMAIN_WOP ..."
            subfinder -d $DOMAIN_WOP | tee -a subfinder.txt
            echo -e "[*] subfinder scan results stored in subfinder.txt \n"
        else
            echo "Error: Unable to retrieve the Domain for subfinder Scan."
        fi

        # Perform subzy scan
        if [ -n "$DOMAIN" ]; then
            echo "[*] Performing subzy scan on $DOMAIN ..."
            subzy run --target $DOMAIN | tee -a subzy.txt
            echo -e "[*] subzy scan results stored in subzy.txt \n"
        else
            echo "Error: Unable to retrieve the Domain for subzy Scan."
        fi

        # Perform aquatone scan
        if [ -n "$DOMAIN" ]; then
        ...
        # Perform eyewitness scan
        if [ -n "$DOMAIN" ]; then
            echo "[*] Performing eyewitness scan on $DOMAIN ..."
            eyewitness -f domain.txt --web -d "eyewitness_results"
